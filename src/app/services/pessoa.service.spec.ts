...
describe('PessoaService', () => {
  ...
  describe('cpfUnicoValidator', () => {
    ...
    it('should return null when control value is empty', (done) => {
      ...
      if (result instanceof Observable) {
        result.subscribe((validationResult: any) => {
          expect(validationResult).toBeNull();
          done();
        });
      } else {
        result.then((validationResult: any) => {
          expect(validationResult).toBeNull();
          done();
        });
      }
    ...
    it('should return null when CPF does not exist', (done) => {
      control.setValue('999.999.999-99');
      const validator = service.cpfUnicoValidator();
      const result = validator(control);
      ...
      setTimeout(() => {
        const req = httpMock.expectOne('api/pessoas?cpf=999.999.999-99');
        req.flush([]);
      }, 600);
    });

    it('should return error when CPF already exists', (done) => {
      control.setValue('123.456.789-00');
      const validator = service.cpfUnicoValidator();
      const result = validator(control);
      ...
      setTimeout(() => {
        const req = httpMock.expectOne('api/pessoas?cpf=123.456.789-00');
        req.flush([mockPessoa]);
      }, 600);
    });
  ...
  describe('URL construction', () => {
    it('should use correct base URL for all operations', () => {
      ...
      service.buscarPorCPF('123.456.789-00').subscribe();
      ...
      const pessoasCpfRequests = httpMock.match(
        'api/pessoas?cpf=123.456.789-00'
      );
